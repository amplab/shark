...
class TCGDate(ec: Int = EnumConstant.NOT_CONST, ref: Date = new Date(12345678l), name: String = "pdate")
  extends TCGPrimitive(ec, name, ref) {

  override def createConstValue() = if (ref == null) null else new DateWritable(ref)

  override def createOI(): OI = {
    if (ec == EnumConstant.CONST) {
      POIF.getPrimitiveWritableConstantObjectInspector(PrimitiveCategory.DATE, createConstValue())
    } else {
      // not constant
      POIF.writableDateObjectInspector
    }
  }

  override def compare(data: AnyRef, oi: OI) {
    assert(oi.isInstanceOf[DateObjectInspector])
    if (ref == null) {
      assert(oi.asInstanceOf[DateObjectInspector].getPrimitiveJavaObject(data) == null)
    } else {
      assert(org.apache.commons.lang.ObjectUtils.equals(ref, oi.asInstanceOf[DateObjectInspector].getPrimitiveJavaObject(data)))
    }
  }
}
