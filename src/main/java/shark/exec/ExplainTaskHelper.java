...
public class ExplainTaskHelper {
  ...
  public static void outputPlan(Serializable work, PrintStream out, boolean extended,
    ...
    // conf and then
    // the children
    if (work instanceof shark.exec.Operator) {
      shark.exec.Operator<org.apache.hadoop.hive.ql.exec.Operator<?>> operator =
          (shark.exec.Operator<org.apache.hadoop.hive.ql.exec.Operator<?>>) work;
      ...
      if (operator.parentOperators() != null) {
        for (shark.exec.Operator op : operator.parentOperatorsAsJavaList()) {
