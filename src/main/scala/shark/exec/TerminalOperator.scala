...
import shark.{CacheKey, RDDUtils, SharkEnv, SharkUtilities}
...
class FileSinkOperator extends TerminalOperator with Serializable {
  ...
  override def execute(): RDD[_] = {
    ...
    rddPreprocessed.context.runJob(rddPreprocessed,
...
object FileSinkOperator {
  def executeProcessFileSinkPartition(op: FileSinkOperator) = {
    ...
    def writeFiles(context: TaskContext, iter: Iterator[_]): Boolean = { 
      val op = SharkUtilities.xmlDeserialize(serializedOp).asInstanceOf[FileSinkOperator]
 ...
 */
class CacheSinkOperator(@BeanProperty var tableName: String) extends TerminalOperator with Serializable {
