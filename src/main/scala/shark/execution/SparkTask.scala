...
with java.io.Serializable with LogHelper {
  ...
  override def execute(driverContext: DriverContext): Int = {
     ...
     * WARN: This function shouldn't be called in high frequency.
     */
    var cc = new CompilationContext(
        SharkConfVars.getBoolVar(Operator.hconf, SharkConfVars.EXPR_CG))
    terminalOp.initializeMasterOnAll(cc)
    
    // TODO this also to refresh the classloader of CGObjectOperator, but from the
    // performance point of view, the classloader of CGObjectOperator should be eliminated
    // Hope spark could provide extra API to intercept the classloader dynamically.
    // otherwise, the generated byte codes will be synced to slaves per task (loaded as 
    // independent classloader instance per task), ideally, there should be only one classloader 
    // instance per worker, which shared among all of the running threads.
    var ocl = cc.compileAll()
