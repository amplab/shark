...
class CGOIPrimitiveDate(delegate: CGPrimitiveDate, order: Int) 
  extends CGOIPrimitive(delegate, order)
...
object CGOIField {
  def create(field: CGField[_]): CGOIField[_ <: CGField[_]] = {
    ...
    field match {
      ...
      case a: CGPrimitiveDate      => new CGOIPrimitiveDate(a, a.order)
  ...
  private[this] val DATA_JAVACLASS = Array[(CGOIField[_]) => String](
    ...
    (f) => f.asInstanceOf[CGOIPrimitiveDate].delegate.clazz,
