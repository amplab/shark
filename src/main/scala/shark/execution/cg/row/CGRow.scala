...
class CGPrimitiveBinary(oi: BinaryObjectInspector, name: String, order: Int)
  ...
  protected[this] override def dataOI2Java(data: Any) = oi.getPrimitiveJavaObject(data).getData()
}
...
class CGPrimitiveDate(oi: DateObjectInspector, name: String, order: Int)
  extends CGPrimitive(oi, name, "java.sql.Date", "java.sql.Date", order) {
}
...
object CGField {
  ...
  private def create(oi: ObjectInspector, name: String): CGField[_ <: ObjectInspector] = {
    ...
    oi match {
      ...
      case x: DateObjectInspector      => new CGPrimitiveDate(x, name, ORDER_P_DATE)
