...
class CGClassEntry(val desc: ExprNodeDesc)
   ...
   */
  def initialize(rowInspector: ObjectInspector) = synchronized ({
    if (!initialized) {
      initialized = true
      try {
...
object CGClassEntry extends LogHelper {
  ...
  private val tuples = new ArrayBuffer[(ExprNodeGenericFuncDesc, CGClassEntry)]()

  private def getRandomClassName() = "GEN" + UUID.randomUUID().toString().replaceAll("\\-", "_")

  def apply(desc: ExprNodeGenericFuncDesc) = synchronized {(
    // TODO no identical Desc found in distributed mode, bug in the de-serialization?
    tuples.find(_._1.isSame(desc)) match {
      case Some(x) => x._2
      case None => {
        var entry = new CGClassEntry(desc)
        tuples += ((desc, entry))
