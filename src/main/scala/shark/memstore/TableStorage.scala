...
class PrunedTableIterator(selStar:Boolean, 
						  outputColumns: Seq[Int],
						  @transient delegate: TableIterator) extends Iterator[ColumnarStruct] {
  
  val struct = new PrunedColumnarStruct(delegate.struct.columnIterators, outputColumns.toArray)
  var position = 0
  val s = delegate.table.size
  
  override def hasNext(): Boolean = position < s
  
  override def next(): ColumnarStruct = {
    position += 1
    struct.nextRow
    struct
  }
  
  class PrunedColumnarStruct(columnIterators: Array[ColumnFormatIterator], columns: Array[Int])
  	extends ColumnarStruct(columnIterators) {
    val sz = columns.size
    
    override def nextRow() = {
      if (selStar.unary_!) {
    	 // outputColumns.foreach(index => columnIterators.apply(index).nextRow)
        var i = 0
        while (i< sz) {
          columnIterators(columns(i)).nextRow
          i += 1
        }
      }
    }
    
    override def getFieldsAsList(): JList[Object] = {
      // columns.map(index => columnIterators(index).current)
      val list = new JArrayList[Object](sz)
      var i = 0 
      while (i < sz) {
        list.add(columnIterators(columns(i)).current)
        i += 1
      }
      list
    }
 ...
 */
class TableIterator(val table: TableStorage) extends Iterator[ColumnarStruct] {
